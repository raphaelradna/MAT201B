...
/* 2. Copy the file _particles-p1.cpp_ to _particles-p2.cpp_. Implement a stable
 * particle system that uses mass relationships like those found in our solar
 * system[^behaviour]. Refer to [List of Solar System objects][]. You do not
 * have to use the exact mass numbers, but there should be only 1 very, very
 * large mass (like the sun), only a few very large masses (like planets), and
 * more medium masses (moons), and many small masses (comets). Use a correct G
